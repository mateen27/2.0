...
const Home = () => {
  ...
  const fetchUsersPost = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        "http://192.168.29.181:3001/api/user/posts"
      );
 ...
 // Fetch posts every time the screen is focused
 useFocusEffect(
    useCallback(() => {
      fetchUsersPost();
    }, [])
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      // Only render the post if the type is 'Reel' or 'image'
      if (item.type === 'reel' || item.type === 'image') {
        return (
          <View key={index} style={{ marginVertical: 5 }}>
            ...
            >
              ...
              <View>
                <Text
                  style={{ fontSize: 15, fontWeight: "600", color: "#f1f1f1" }}
                >
                  {item?.userID?.name}
                </Text>
                {/* <Text
      ...
      // If the post type is not 'Reel' or 'image', skip rendering
      return (
        <View key={index} style={{ marginVertical: 5 }}>
            {/* View for the profile information like name , bio , profileImage , dateOfPost */}
            <View
